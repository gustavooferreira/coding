...
func (s *Solver) ComputeResult() {
	...
	if s.debug {
		fmt.Println("Hands:")
		for i, hand := range s.hands {
			fmt.Printf("%4d - Representation: %s -- Bid: %4d -- Type: %s\n", i+1, hand.HandRepresentation, hand.Bid, hand.Type)
		}
	}
...
func computeCardHandStrength(cardHand string) HandType {
	...
	for cardType, count := range cardTypeCount {
		if cardType == 'J' {
			continue
		}
	...
	if foundFourCount == 1 {
		result = HandType_FourOfAKind
	} else if foundThreeCount == 1 && foundTwoCount == 1 {
		result = HandType_FullHouse
	} else if foundThreeCount == 1 {
		result = HandType_ThreeOfAKind
	} else if foundTwoCount == 2 {
		result = HandType_TwoPair
	} else if foundTwoCount == 1 {
		result = HandType_OnePair
	} else {
		result = HandType_HighCard
	}

	result = handPromotion(result, cardTypeCount['J'])

	return result
}

// for each hand, include the next best hand depending on how many jokers we have
var handPromotionRank = map[HandType]HandType{
	HandType_HighCard:     HandType_OnePair,
	HandType_OnePair:      HandType_ThreeOfAKind,
	HandType_TwoPair:      HandType_FullHouse,
	HandType_ThreeOfAKind: HandType_FourOfAKind,
	HandType_FourOfAKind:  HandType_FiveOfAKind,
	HandType_FiveOfAKind:  HandType_FiveOfAKind, // this is only to handle an edge case.
}

func handPromotion(inputHand HandType, jokersCount int) HandType {
	result := inputHand

	for i := 0; i < jokersCount; i++ {
		result = handPromotionRank[result]
	}
