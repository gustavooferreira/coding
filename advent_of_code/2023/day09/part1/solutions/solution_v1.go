...
func (s *Solver) ComputeResult() {
	...
	for _, line := range s.puzzle {
		prediction := s.predictNextNumber(line)
		result += prediction
	}
...
func (s *Solver) predictNextNumber(line []int) int {
	...
	// compute prediction
	for i := len(intermediaryLines) - 1; i >= 0; i-- {
		lastIndex := len(intermediaryLines[i]) - 1
		result += intermediaryLines[i][lastIndex]
	}
